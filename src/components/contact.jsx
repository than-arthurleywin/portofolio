...
function Contact() {
  ...
  const sendEmail = (e) => {
    ...
    emailjs
      .sendForm(
        "service_s2hmsfb", // Service ID dari EmailJS
        "template_4cpvg2k", // Template ID dari EmailJS
        form.current,
        "ot-snIG3HaKvciO_4" // Public Key dari EmailJS
      )
      .then(
        (result) => {
          alert("Pesan berhasil dikirim!");
          console.log(result.text); // Debug jika berhasil
        },
        (error) => {
          alert("Terjadi kesalahan, coba lagi.");
          console.error(error.text); // Debug jika gagal
        }
      );
  };
